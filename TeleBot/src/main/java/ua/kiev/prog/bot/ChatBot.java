...
public class ChatBot extends TelegramLongPollingBot {
    ...
    private static final String REMOVE_USERS = "remove ";
    ...
    private boolean checkIfAdminCommand(User user, String text) {
        ...
        if(text.startsWith(REMOVE_USERS)){
            LOGGER.info("Admin command received: " + REMOVE_USERS);
            text = text.substring(REMOVE_USERS.length());
            removeUsers(text);
            return true;
        }
    ...
    private void removeUsers(String text){
        List<User> users = userService.findAllUsers();
        users.forEach(user -> {
